"""Tests for load_runs()."""
import os
import sys
import tempfile
from pathlib import Path
import pytest

sys.path.insert(0, str(Path(__file__).parent.parent.parent))

from util import load_runs

_HEADER = "skip,tc0,tc1,tt0,tt1,tau_u,tau_s,rho,omega,m,chi,alpha,mu_type,label,epsilon,beta,bloc\n"


def _load(content: str):
    """Write content to a temp CSV, load_runs() it, and clean up."""
    with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
        f.write(content)
        tmp = Path(f.name)
    try:
        return load_runs(tmp)
    finally:
        os.unlink(tmp)


def test_all_rows_loaded():
    """skip=1 rows are excluded; skip=0 rows are loaded."""
    content = _HEADER + (
        "0,2020,2024,2020,2024,10,10,0,0,0110,0.5,1.0,,baseline,0,0,\n"
        "1,2020,2024,2020,2024,10,10,0,0,0110,0.5,1.0,,skipped,0,0,\n"
        "0,2020,2024,2020,2024,20,20,0,0,1111,0.5,1.0,,full-joint,0,0,\n"
    )
    runs = _load(content)
    assert len(runs) == 2
    assert [r.label for r in runs] == ['baseline', 'full-joint']


def test_skip_filters():
    content = (
        "skip,run_code,tc0,tc1,tt0,tt1,fx,tau_u,tau_s,rho,m,chi,alpha,mu_type,label\n"
        "0,20242024,2020,2024,2020,2024,A,20,20,0,0110,0.5,1.0,,baseline\n"
        "1,20242024,2020,2024,2020,2024,E,20,20,0,0110,0.5,1.0,,F=E\n"
    )
    runs = _load(content)
    assert len(runs) == 1
    assert runs[0].label == 'baseline'


def test_types():
    content = _HEADER + "0,2020,2024,2020,2024,10,10,0,0,0110,0.5,1.0,,baseline,0,0,\n"
    r = _load(content)[0]
    assert isinstance(r.tc0,   int),   f"tc0 should be int, got {type(r.tc0)}"
    assert isinstance(r.tc1,   int),   f"tc1 should be int, got {type(r.tc1)}"
    assert isinstance(r.tt0,   int),   f"tt0 should be int, got {type(r.tt0)}"
    assert isinstance(r.tt1,   int),   f"tt1 should be int, got {type(r.tt1)}"
    assert isinstance(r.tau_u, float), f"tau_u should be float, got {type(r.tau_u)}"
    assert isinstance(r.tau_s, float), f"tau_s should be float, got {type(r.tau_s)}"
    assert isinstance(r.rho,   int),   f"rho should be int, got {type(r.rho)}"
    assert isinstance(r.chi,   float), f"chi should be float, got {type(r.chi)}"
    assert isinstance(r.alpha, float), f"alpha should be float, got {type(r.alpha)}"
    assert isinstance(r.m,     tuple), f"m should be tuple, got {type(r.m)}"
    assert isinstance(r.label, str),   f"label should be str, got {type(r.label)}"


def test_chi_star_is_float():
    content = _HEADER + (
        "0,2020,2024,2020,2024,10,10,0,0,1111,-1.0,1.0,,full-joint-chi-star,0,0,\n"
        "0,2020,2024,2020,2024,10,10,0,0,0110,0.5,1.0,,baseline,0,0,\n"
    )
    runs = _load(content)
    chi_star_rows = [r for r in runs if r.chi == -1.0]
    assert len(chi_star_rows) == 1
    assert chi_star_rows[0].label == 'full-joint-chi-star'
    assert isinstance(chi_star_rows[0].chi, float)


def test_m_is_tuple():
    """m is parsed from the CSV's '0110' string into a tuple of ints."""
    content = _HEADER + "0,2020,2024,2020,2024,10,10,0,0,0110,0.5,1.0,,baseline,0,0,\n"
    r = _load(content)[0]
    assert r.m == (0, 1, 1, 0)


def test_invalid_m_raises():
    content = _HEADER + "0,2020,2024,2020,2024,10,10,0,0,0011,0.5,1.0,,bad-m,0,0,\n"
    with pytest.raises(ValueError, match='invalid m'):
        _load(content)
