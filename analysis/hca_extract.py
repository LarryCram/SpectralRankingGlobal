...
Produces WORKING/hca_authors.parquet: one row per (author_idx × field_idx),
covering every OAX author who appears on at least one HCW in that field.
hca_match.py reads this file and applies a per-field threshold before clustering.
...
HCW definition:
  Top 1% of cited_by_count per (field_idx, publication_year) within hca_flat_works.
...
Per-field threshold (applied in hca_match.py, not here):
  ...
  Keeps roughly the top 10% most prolific HCW authors per field; varies from
  n=2 (small fields: Maths, Arts) to n=3 (large fields: Medicine, Engineering).

Pipeline (each file built once, skipped if present):
  1. WORKING/filtered_works_topics.parquet  — (work × topic), with field_idx
  2. WORKING/hca_flat_works.parquet         — (work × field), with field_share
  3. WORKING/hca_hcw.parquet               — top-1% HCW per (field × year)
  4. WORKING/hca_authors.parquet           — (author × field) with metadata

Output schema (hca_authors.parquet):
  ...
  row_idx is a stable 1-based integer key used by hca_match.py as the name_idx row key.
...
# ── Stage 2: hca_flat_works ───────────────────────────────────────────────────

def build_hca_flat_works(topics_path: Path, out: Path) -> None:
    """
    ...
    if out.exists():
        n = (duckdb.execute(f"SELECT COUNT(*) FROM '{out}'").fetchone() or (0,))[0]
        print(f'  hca_flat_works exists  ({n:,} rows) — skipping')
        return

    print('  Building hca_flat_works.parquet ...')
    t0 = time.time()
    con = _con(out.parent)
    con.execute(f"""
    COPY (
    ...
    n = (duckdb.execute(f"SELECT COUNT(*) FROM '{out}'").fetchone() or (0,))[0]
    w = (duckdb.execute(f"SELECT COUNT(DISTINCT work_idx) FROM '{out}'").fetchone() or (0,))[0]
    print(f'  hca_flat_works: {n:,} rows, {w:,} works  [{time.time()-t0:.0f}s]')


# ── Stage 3: hca_hcw ─────────────────────────────────────────────────────────

def build_hca_hcw(flat_path: Path, out: Path) -> None:
    """
    Top-1% of cited_by_count per (field_idx, publication_year) from hca_flat_works.
    ...
    if out.exists():
        n = (duckdb.execute(f"SELECT COUNT(*) FROM '{out}'").fetchone() or (0,))[0]
        w = (duckdb.execute(f"SELECT COUNT(DISTINCT work_idx) FROM '{out}'").fetchone() or (0,))[0]
        print(f'  hca_hcw exists  ({n:,} rows, {w:,} works) — skipping')
        return

    print('  Building hca_hcw.parquet ...')
    t0 = time.time()
    con = _con(out.parent)
    con.execute(f"""
    COPY (
      ...
      WHERE fw.cited_by_count >= th.p99
    ) TO '{out}' (FORMAT PARQUET)
    """)
    con.close()
    n = (duckdb.execute(f"SELECT COUNT(*) FROM '{out}'").fetchone() or (0,))[0]
    w = (duckdb.execute(f"SELECT COUNT(DISTINCT work_idx) FROM '{out}'").fetchone() or (0,))[0]
    print(f'  hca_hcw: {n:,} rows, {w:,} works  [{time.time()-t0:.0f}s]')


# ── Stage 4: hca_authors ──────────────────────────────────────────────────────

MAX_AUTHORS = 30   # exclude HCW with > 30 authors (mirrors Clarivate)


def build_hca_authors(hcw_path: Path, oax: Path, out: Path) -> None:
    """
    Join HCW to authorships; aggregate to (author × field) with metadata.
    Works with > MAX_AUTHORS authors are excluded before counting.
    Skipped if the file already exists.
    """
    if out.exists():
        n = (duckdb.execute(f"SELECT COUNT(*) FROM '{out}'").fetchone() or (0,))[0]
        print(f'  hca_authors exists  ({n:,} rows) — skipping')
        return

    print('  Building hca_authors.parquet ...')
    t0 = time.time()
    con = _con(out.parent)
    ...
    SELECT work_idx FROM (
        SELECT work_idx, COUNT(DISTINCT author_idx) AS n
        FROM _auth_raw GROUP BY work_idx
    ) WHERE n <= {MAX_AUTHORS}
    """)

    con.execute("""
    CREATE TEMP TABLE _auth AS
    SELECT r.* FROM _auth_raw r JOIN _keep k ON k.work_idx = r.work_idx
    """)

    con.execute("""
    CREATE TEMP TABLE _counts AS
    SELECT field_idx, author_idx, COUNT(DISTINCT work_idx) AS n_hcw
    FROM _auth GROUP BY field_idx, author_idx
    """)
    ...
    WITH _ic AS (
        ...
        FROM _auth WHERE institution_name IS NOT NULL
    ...
    print(f'  hca_authors: {n:,} rows, {a:,} distinct authors  [{time.time()-t0:.0f}s]')
...
def main() -> None:
    ...
    flat_path   = w / 'hca_flat_works.parquet'
    hcw_path    = w / 'hca_hcw.parquet'
    out_path    = w / 'hca_authors.parquet'

    print('Stage 1: filtered_works_topics')
    build_filtered_works_topics(oa, topics_path)

    print('Stage 2: hca_flat_works')
    build_hca_flat_works(topics_path, flat_path)

    print('Stage 3: hca_hcw  (top 1% per field × year)')
    build_hca_hcw(flat_path, hcw_path)

    print('Stage 4: hca_authors')
    build_hca_authors(hcw_path, oa, out_path)
    ...
    n, a, f = duckdb.execute(f"""
        ...
        FROM '{out_path}'
    """).fetchone()
    print(f'  {n:,} (author × field) rows, {a:,} distinct authors, {f} fields')
